...
# vectorised_timeseries
{
  ...
  time_grid <- tibble(
    ...
    filter(
      !(day_of_week %in% excluded_days) # excluded_days defined in wrapper
    ) |>
    dplyr::select(
      !day_of_week
    )
...
save(
  ...
  file = output_file # defined in wrapper
)


cat("\n\nDone writing to", output_file)
