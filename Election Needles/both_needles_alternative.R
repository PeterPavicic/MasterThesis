...
text(x = as.POSIXct("2024-11-06 01:08", tz = "America/New_York"),
     y = 0.7,
     labels = "Decision Desk HQ\ncall election",
     col = "purple",
     adj = 1)
text(x =  as.POSIXct("2024-11-06 02:01", tz = "America/New_York"),
     y = 0.7,
     labels = "Fox News\ncall election",
     col = "purple",
     adj = 0)
...
# For NY Times needle consider last available timestamp:
get_latest <- function(timestamp, df) {
  latest <- df[df$time <= timestamp, ]
  if (nrow(latest) == 0) {
    return(NA) # Return NA if no prior timestamps
  }
  return(latest$odds[nrow(latest)])
}

pm_timestamps <- polymarket_trump_needle$Timestamp[polymarket_trump_needle$Timestamp <= winner_called]
nyt_trump_odds <- ts(sapply(pm_timestamps, get_latest, nyt_trump_needle))
...
johansen_test <- ca.jo(cbind(pm_trump_odds, nyt_trump_odds), type = "trace",
                       K = var_select$selection["HQ(n)"], ecdet = "trend")
...
granger_test <- causality(var_model, cause = "pm_needle_diff")  # Test if Polymarket predicts NYT needle
